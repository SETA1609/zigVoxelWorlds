...
- **World editability policy** — Voxel mutations are **not** automatically allowed. Each scene/region declares an **edit policy** in TOML; `VoxelServer` evaluates it before applying any delta. Policies are composable (project default → scene → sub-region) and resolve to one of: `full` (anything goes — Minecraft mode), `none` (read-only — typical for town centers, cutscene areas), `voxel_type_allowlist` (e.g. only ore + dirt destructible — Stardew mines), `coord_range_allowlist` (only inside an AABB — designated farming plots, dungeon rooms), `tag_allowlist` (voxels tagged `destructible` only — barrels in houses, crates in dungeons), `script` (defer to a script callback for arbitrary logic — quest-gated areas, time-of-day rules). The policy lives in scene/region TOML so each target game (Daggerfall clone read-only world / Stardew farm + mines / Atelier gathering zones / rogue-like dungeons) configures it without engine code changes.
