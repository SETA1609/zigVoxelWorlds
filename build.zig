...
const dynamic_libs = [_][]const u8{};

const project_name = "MyDemoProject";
// --- Source discovery -----------------------------------------------------
...
// adds libraris to the compile code
fn addLibs(exe_mod: *std.Build.Module) void {
    ...
    if (static_libs.len > 0) {
        for (static_libs) |lib| {
            // add static_libs by lib

        }
    }

    if (dynamic_libs.len > 0) {
        for (dynamic_libs) |lib| {
            // add dy   dynamic_libs by lib
        }
    }
}
...
pub fn build(b: *std.Build) void {
    ...
    const c_sources = getFilesFromDir(b.graph.io, b.allocator, c_dir, &.{c_suffix}) catch |err|
        std.debug.panic("Failed to scan {s}: {s}", .{ c_dir, @errorName(err) });
    const cpp_sources = getFilesFromDir(b.graph.io, b.allocator, cpp_dir, &.{cpp_suffix}) catch |err|
        std.debug.panic("Failed to scan {s}: {s}", .{ cpp_dir, @errorName(err) });
