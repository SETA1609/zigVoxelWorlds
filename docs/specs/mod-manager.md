...
### Mod SDK workflow — requires the parent game

Per [`engine-vs-game.md` § 3b Mod project + § Modkit](../engine-vs-game.md), mods target a *specific game*, not the engine. The full workflow:

1. **Modder installs the shipped game.** The launcher binary holds the baked `PROJECT_PUBKEY` and the runtime that hosts the WASM sandbox.
2. **Modder installs the modkit.** Generated by the game's export step alongside the game binary, ships free with the game. Contains: `modkit.toml`, `headers/`, `sample_mod/`, content-ID list — see [`engine-vs-game.md` § Modkit](../engine-vs-game.md).
3. **Modder opens the modkit in the engine editor** (or a standalone SDK build of the editor). The editor switches to "New Mod Project" flow: parent game's content shown **read-only** for reference; modder's own data shown **writable, additive on top**.
4. **Modder writes their mod** in Zig / C++ / Rust / AssemblyScript (anything that compiles to WASM). The mod consumes the game's published C ABI subset declared in `modkit.toml`.
5. **Build to WASM.** The editor's mod-build pipeline targets `wasm32-freestanding` (or `wasm32-wasi` if the mod opts into WASI sub-features — note: full WASI is never exposed in the sandbox, only the curated Host API).
6. **Test by running the shipped game** with `--mods=<their-mod-dir>` (or via the in-game mod manager). The launcher loads the mod into its WASM sandbox. The mod is NOT signed by the publisher, so it falls back to WASM tier as designed — exactly how a final player would experience it.
7. **Publish** to Steam Workshop or direct download. End users' game launchers load it sandboxed.

The mod-build pipeline + the `--mods` flag are part of the *game's* runtime, not the engine's editor in isolation. The engine alone doesn't know about any specific game; the SDK workflow is bound to the parent game install.

### Proprietary IP boundary — hard rule

Modders MUST NOT be able to read the game's source code through the modkit or the shipped game install. See [`engine-vs-game.md` § Proprietary IP boundary](../engine-vs-game.md) for the full artifact table. Summary:

- **Game source (`.zig` / `.cpp` / `.h`)** — never shipped anywhere
- **Compiled binary + PCK** — shipped to run the game; opaque to modders (PCK TOML data IS readable, intentionally — modders need to reference content IDs)
- **Modkit** — public interface only: headers, ID list, sample mod, schema. No game logic.

The export step at [`src/editor/export/`](../../src/editor/export/README.md) MUST enforce this with a build-time check — if any source file leaks into a packaged artifact, the export fails loudly.
